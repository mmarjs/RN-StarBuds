...
class OtherProfile extends Component {
  ...
  async getUserDetailsFromApi() {
    ...
    console.log('this.apiHeaders', this.apiHeaders);
    ...
    apiCall("users/getUserDetails", data, headers)
      .then(response => {
        if (page == 0) {
          ...
          this.setState({ posts: tempData, refreshing: false, isLoading: false });
        }
  ...
  getUserDetails () {
    // make request to get userDetails
    const data = {
      userId: this.props.userData._id,
        ...
        >
          {this.state.followedOrNot ? (
            <Image
              ...
              style={[ Styles.headerRightImage, { height: 25, width: 25 }]}
            />
          ) : (
            <Image
  ...
  renderProfileImage() {
    return (
      <View style={OtherProfileStyle.profileImageContainer}>
        <CachedImage
  ...
  renderChatMessage() {
    return (
      <View style={OtherProfileStyle.findBudsContainer}>
        <Button
          disabled={this.state.disableGotoChat}
          onPress={() => {
            this.addChat();
          }}
          style={{ backgroundColor: "#3D3E3E", height: 30 }}
        >
