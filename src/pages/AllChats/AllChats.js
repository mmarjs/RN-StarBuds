...
class AllChats extends Component {
  ...
  renderHeader = () => {
    return (
      <Input
        ...
        style={[{backgroundColor : 'rgb(243, 244, 246)',marginRight : 20, marginLeft : 20,borderRadius : 10, height : 43, marginTop : 5 }, Styles.thinBottomBorder7]}
  ...
  renderListItem = ({ item, index }) => {
    ...
    let fontColor = item.readStatus === false ? {color:Colors.black} : {}
    return (
      <TouchableOpacity
      ...
      <View style={AllChatsStyle.flatItemContainer}>
        ...
        <View style={AllChatsStyle.chatDetail} >
          <View>
            ...
            <Text style={[{color:Colors.dark},fontStyle,fontColor]} numberOfLines={2} ellipsizeMode={'tail'} >
              {item.lastMessage}
            </Text>
          </View>
        </View>
  ...
  render(){
    ...
    return(
      <View style={AllChatsStyle.container}>
        ...
        {searchText.length == 0 && filteredChatsByName.length === 0 && !this.state.loading && (
        <View style={AllChatsStyle.emptyContainer }>
          <Image style={{width : '100%', height : screenHeight * 0.93,marginTop:-screenHeight * 0.06}}
...
AllChats.navigationOptions = ({ navigation }) => ({
  ...
  headerStyle: Styles.headerStyle,
    ...
    headerRight: (
      ...
      >
        <Image style={[Styles.headerRightImage, {tintColor : 'black', height: 15, width: 15 }]} source={Images.addNewChat}/>
      </TouchableOpacity> 
    ),
  tabBarVisible: false,
});
