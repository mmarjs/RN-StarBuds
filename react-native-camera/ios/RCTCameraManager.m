    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:[self.stillImageOutput connectionWithMediaType:AVMediaTypeVideo] completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
            ...
            if (imageDataSampleBuffer) {
                ...
                UIImage *testImage = [UIImage imageWithData:rotatedImageData];                
                CGRect rect = [self.previewLayer metadataOutputRectOfInterestForRect: self.previewLayer.bounds];
                
                CGImageRef sourceRef = [testImage CGImage];
                NSUInteger width = CGImageGetWidth(sourceRef);
                NSUInteger height = CGImageGetHeight(sourceRef);
                CGRect cropRect = CGRectMake(rect.origin.x * width, rect.origin.y * height, rect.size.width * width, rect.size.height * height);
                CGImageRef imageRef = CGImageCreateWithImageInRect([testImage CGImage], cropRect);
                
                UIImage *imageCropped = [UIImage imageWithCGImage:imageRef scale:1.0 orientation:testImage.imageOrientation];
                CGImageRelease(imageRef);
                UIImage* rotateImg;
                if ([imageCropped imageOrientation] != UIImageOrientationUp){
                    UIGraphicsBeginImageContextWithOptions(imageCropped.size, false, imageCropped.scale);
                    [imageCropped drawInRect:CGRectMake(0, 0, imageCropped.size.width, imageCropped.size.height)];
                    rotateImg = UIGraphicsGetImageFromCurrentImageContext();
                    UIGraphicsEndImageContext();
                }else{
                    rotateImg = imageCropped;
                }
                [self saveImage:[UIImageJPEGRepresentation(rotateImg, 1.0) mutableCopy] target:target metadata:imageMetadata resolve:resolve reject:reject];
...
/*
 ...
 NSNumber* orientationValue = [img valueForKey:@"ALAssetPropertyOrientation"];
 if (orientationValue) {
